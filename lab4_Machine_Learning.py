...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    num_ratings = len(IDandRatingsTuple[1])
    avg_ratings = float(sum(IDandRatingsTuple[1]))/num_ratings
    return (movie_id, (num_ratings, avg_ratings))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda a: ((a[0], a[1]), a[2]))
    ...
    totalError = squaredErrorsRDD.map(lambda a: math.pow((float(a[1][0])-float(a[1][1])),2)).reduce(lambda a,b: a+b)
...
trainingAvgRating = trainingRDD.map(lambda a: a[2]).reduce(lambda a, b: a+b) * 1.0 / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda a: (a[0], a[1], trainingAvgRating))
...
myUnratedMoviesRDD = moviesRDD.leftOuterJoin(myRatingsRDD.map(lambda a: (a[1], a[0]))).filter(lambda a: a[1][1]==None).map(lambda a: (0, a[0]))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda a: (a[0], a[1][0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda a: (a[1], a[2]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda a: (a[1][0][0], a[1][1], a[1][0][1])).filter(lambda a: a[2] > 75))
