...
def tokenize(string):
    ...
    """
    out = []
    for a in simpleTokenize(string):
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda a: (a[0], tf(a[1]).keys())).flatMap(lambda a: a[1])
    tokenCountPairTuple = uniqueTokens.map(lambda a: (a, 1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a+b)
    return (tokenSumPairTuple.map(lambda a: (a[0], (N * 1.0) /a[1])).sortByKey(lambda a: -a[1]))
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda a: ('%s %s' % (a[1], a[0]), float('%f' % a[2])))

trueDupsRDD = (sims.join(goldStandard))

trueDupsCount = trueDupsRDD.count()

avgSimDups = trueDupsRDD.map(lambda a: a[1][0]).reduce(lambda x, y: x+y) / trueDupsCount

nonDupsRDD = (sims.leftOuterJoin(goldStandard).filter(lambda x: x[1][1]==None)).map(lambda x: (x[0], x[1][0]))

avgSimNon = (nonDupsRDD.map(lambda x: x[1]).reduce(lambda x, y: x+y)) / (nonDupsRDD.count())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda a: (a[0], tokenize(a[1])))
googleFullRecToToken = google.map(lambda a: (a[0], tokenize(a[1])))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda a: (a[0], tfidf(a[1], idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda a: (a[0], tfidf(a[1], idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda a: (a[0], norm(a[1]))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda a: (a[0], norm(a[1]))).collectAsMap()
...
def fastCosineSimilarity(record):
   ...
   """
    ...
    s = 0
    for k in tokens:
        s += amazonWeightsBroadcast.value[amazonRec][k] * googleWeightsBroadcast.value[googleRec][k]
