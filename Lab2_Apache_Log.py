...
dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.date().day, log.host))

dayGroupedHosts = dayToHostPairTuple.distinct().groupByKey().sortByKey()

dayHostCount = dayGroupedHosts.map(lambda a: (a[0], len(a[1])))
...
dayAndHostTuple = access_logs.map(lambda log: (log.date_time.date().day, log.host))

groupedByDay = dayAndHostTuple.groupByKey().map(lambda a: (a[0], len(a[1])))

sortedByDay = groupedByDay.join(dailyHosts).sortByKey()

avgDailyReqPerHost = (sortedByDay
                      .map(lambda a: (a[0], a[1][0]/a[1][1]))).cache()
...
hourCountPairTuple = badRecords.map(lambda log: (log.date_time.time().hour, 1))
